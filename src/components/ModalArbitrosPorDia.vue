  ...
  <div style="display: flex; flex-direction: column; gap: 1.5rem">
    ...
    >
      <div style="display: flex; align-items: center; gap: 10px">
        <i
          class="ti ti-users"
          style="font-size: 24px; color: var(--color-primary)"
        ></i>
        <div>
          <h3
            style="
          ...
          <div
            ...
            "
          >
            Resumen de árbitros asignados para el fin de semana (excluye
            finalizadas)
          </div>
        </div>
      </div>
      ...
      >
        <i
          class="ti ti-x"
          style="font-size: 20px; color: var(--color-text-secondary)"
        ></i>
      </button>
    </div>
    ...
    <div v-else style="display: flex; flex-direction: column; gap: 1.5rem">
      ...
      >
        ...
        >
          ...
          <div style="font-size: 9px; color: #1e3a8a; margin-top: 2px">
            Sáb. y Dom.
          </div>
        </div>
        <div
        ...
        >
          ...
          <div style="font-size: 9px; color: #065f46; margin-top: 2px">
            Sólo Sáb.
          </div>
        </div>
        <div
      ...
      >
        ...
        >
            ...
            <div style="display: flex; gap: 4px">
              <template v-if="activeTab !== 'sinDesignar'">
                <span
                  v-if="
                    arb.saturday &&
                    arb.saturday.some((m) => m.estadoDesignacion !== 3)
                  "
                  class="badge"
                  style="
                    font-size: 8px;
                ...
                <span
                  v-if="
                    arb.sunday &&
                    arb.sunday.some((m) => m.estadoDesignacion !== 3)
                  "
                  class="badge"
                  style="
                    font-size: 8px;
                ...
                >
                <span
                  v-if="hasCancelledMatches(arb) && !hasActiveMatches(arb)"
                  class="badge"
                  style="
                    font-size: 8px;
                    background: #fef2f2;
          ...
          <div
            ...
            "
          >
            🚫 <strong>Sanción activa:</strong> El árbitro tiene una suspensión vigente.
          </div>

          <!-- Mensaje estado para cancelaciones -->
          <div
            v-if="
              activeTab !== 'sinDesignar' &&
              hasCancelledMatches(arb) &&
              !hasActiveMatches(arb)
            "
            style="
              padding: 6px 8px;
              border-radius: 4px;
            ...
            "
          >
            ❌ <strong>No dirige este fin de semana:</strong> Todas sus
            designaciones fueron canceladas.
          </div>

          <!-- Assignments Details (if assigned) -->
          <div
            v-if="activeTab !== 'sinDesignar'"
            style="
              background: var(--color-background-secondary);
              border-radius: 6px;
              padding: 8px 10px;
          ...
          >
            <div
              v-if="arb.saturday && arb.saturday.length > 0"
            ...
            >
              ...
              >
                <li
                  v-for="match in arb.saturday"
                  :key="match.id"
                  :style="
                    match.estadoDesignacion === 3
                ...
                >
                  ...
                  <span
                    v-if="match.estadoDesignacion === 3"
                    class="badge"
                    style="
                      font-size: 8px;
                      background: #fee2e2;
            ...
            <div
              v-if="arb.sunday && arb.sunday.length > 0"
              style="margin-bottom: 6px"
            >
              <div
                style="
          ...
          >
            ...
            <div style="display: flex; gap: 8px; margin-top: 4px">
              <span
                class="badge"
                :style="
                  arb.disponibleSabado
                    ? 'background: #e1f5ee; color: #0f6e56'
                    : 'background: #f1f5f9; color: #94a3b8'
                "
                style="padding: 1px 6px; border-radius: 6px"
              >
...
onMounted(async () => {
  ...
  try {
    const promises = [];
    if (state.designacionesIncompletas.length === 0)
      promises.push(loadDesignacionesIncompletas());
    if (state.designaciones.length === 0)
      promises.push(loadDesignacionesCompletas());
    if (state.designacionesCanceladas.length === 0)
      promises.push(loadDesignacionesCanceladas());
  ...
  allDesignaciones.value.forEach((d) => {
    ...
    assigned.forEach((asg) => {
      ...
      const matchDetail = {
        ...
        estadoDesignacion:
          d.estadoDesignacion !== undefined ? d.estadoDesignacion : d.estado,
      };
...
const noDirigen = computed(() => {
  return arbitrosResumen.value.filter((a) => {
    const hasCancelled =
      (a.saturday && a.saturday.some((m) => m.estadoDesignacion === 3)) ||
...
const hasCancelledMatches = (arb) => {
  return (
    (arb.saturday && arb.saturday.some((m) => m.estadoDesignacion === 3)) ||
...
const tabs = computed(() => {
  ...
  list.push({
    id: "sinDesignar",
    label: "Sin Designar",
    count: sinDesignar.value.length,
  });
  return list;
});
...
const currentList = computed(() => {
  let list = [];
  if (activeTab.value === "ambos") list = ambosDias.value;
  else if (activeTab.value === "sabado") list = soloSabado.value;
  else if (activeTab.value === "domingo") list = soloDomingo.value;
  else if (activeTab.value === "noDirigen") list = noDirigen.value;
  else if (activeTab.value === "sinDesignar") list = sinDesignar.value;
  else list = arbitrosResumen.value;
